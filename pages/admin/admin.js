$(document).ready(() => {
  ...
  $("#storeGrid").jsGrid({
    ...
    fields: [
      {
        ...
        validate: "required",
        validate: {
          message: "Enter the valid store name",
          validator: (value) => {
      ...
      },
      {
        ...
        width: 60,
        validate: {
          message: "Enter the valid owner name",
          validator: (value) => {
            return /^[a-zA-Z0-9\s]+$/.test(value);
          },
        },
      },
      {
        ...
        title: "Store Email",
        validate: {
          message: "Enter the valid mail id",
          validator: (value) => {
    ...
    controller: {
      loadData: function (filter) {
        return $.grep(adminData.stores, function (store) {
          return (
            (!filter.name ||
              store.name.toLowerCase().indexOf(filter.name.toLowerCase()) >
              -1) &&
            (!filter.storeId ||
              store.storeId
                .toLowerCase()
                .indexOf(filter.storeId.toLowerCase()) > -1) &&
            (!filter.address ||
              store.address
                .toLowerCase()
                .indexOf(filter.address.toLowerCase()) > -1) &&
            (!filter.type ||
              store.type.toLowerCase().indexOf(filter.type.toLowerCase()) >
              -1) &&
            (!filter.ownerName ||
              store.ownerName
                .toLowerCase()
                .indexOf(filter.ownerName.toLowerCase()) > -1) &&
            (!filter.contactEmail ||
              store.contactEmail
                .toLowerCase()
                .indexOf(filter.contactEmail.toLowerCase()) > -1)
          );
        });
      },
    },
    onItemInserted: (node) => {
      if (node.grid.name === "stores") {
        ...
        adminData.stores.some((item) => {
          if (item.storeId.toLowerCase() === node.item.storeId.toLowerCase()) {
    ...
    onItemDeleted: (node) => {
      if (node.grid.name === "stores") {
        for (let i = 0; i < adminData.stores.length; i++) {
          if (adminData.stores[i].storeId.toLowerCase() === node.item.storeId.toLowerCase()) {
  ...
  $("#ownerGrid").jsGrid({
    ...
    controller: {
      loadData: function (filter) {
        return $.grep(adminData.owners, function (owner) {
          return (
            (!filter.storeName ||
              owner.storeName
                .toLowerCase()
                .indexOf(filter.storeName.toLowerCase()) > -1) &&
            (!filter.storeId ||
              owner.storeId
                .toLowerCase()
                .indexOf(filter.storeId.toLowerCase()) > -1) &&
            ...
            (!filter.password ||
              owner.password
                .toLowerCase()
                .indexOf(filter.password.toLowerCase()) > -1) &&
            (!filter.contactEmail ||
              owner.contactEmail
                .toLowerCase()
                .indexOf(filter.contactEmail.toLowerCase()) > -1)
    ...
    onItemDeleted: (node) => {
      if (node.grid.name === "owners") {
        for (let i = 0; i < adminData.owners.length; i++) {
          if (adminData.owners[i].storeId.toLowerCase() === node.item.storeId.toLowerCase()) {
